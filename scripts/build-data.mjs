...
const HTML = path.join(HERE, "..", "data", "headline.js");
...
const DATA = block ? new Function(block[1] + "; return DATA;")() : {};
...
const out =
  ...
  "window.SERIES = " + JSON.stringify(payload, null, 1) + ";\n";

if (DRY) {
  console.log(`\n--dry-run: would write ${(out.length / 1024).toFixed(0)} KB to data/series.js`);
} else {
  fs.mkdirSync(path.dirname(OUT), { recursive: true });
  fs.writeFileSync(OUT, out);
  console.log(`\nWrote data/series.js (${(out.length / 1024).toFixed(0)} KB).`);
}
